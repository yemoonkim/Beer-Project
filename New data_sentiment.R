...
dfmat_corp_review = new.df$text %>% corpus() %>% 
  tokens( what = "word",
          remove_punct = TRUE,
          remove_url=TRUE,
          remove_symbols = TRUE) %>% 
...
new2.df <- cbind(new.df,sentiment_by_tweet)

write.csv(new2.df, "new data set with sentiment.csv")
...
review.df[grepl("beer",review),.(review, ave_sentiment)][order(-ave_sentiment)] %>% head(10)

review.df[grepl("Aung San Suu Kyi",review),.(review, ave_sentiment)][order(-ave_sentiment)] %>% head(10)

review.df[grepl("Aung San Suu Kyi",review),.(review, ave_sentiment)][order(-ave_sentiment)] %>% tail(10)

review.df[grepl("myanmar",review),.(review, ave_sentiment)][order(-ave_sentiment)] %>% tail(10)
